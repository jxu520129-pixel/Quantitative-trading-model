...
历史日线**主数据源为 Tushare**（付费代理更稳定，`.env` 配 `TUSHARE_TOKEN` + 可选 `TUSHARE_API_URL`，留空走官方 `api.tushare.pro`），**备用源为 AkShare**（依赖免费新浪接口，高频拉取易被 SSL 重置/封禁，故仅作回退）；未配置 Tushare Token 时自动退回 AkShare。**实时行情（盘中买点扫描、盘前集合竞价）固定用 AkShare 拉取**——Tushare 不提供实时行情，系统每次仅 1 次请求拉全市场快照、每天约 6 次，低频不触发封禁。**注意数据单位口径**：Tushare 的 `amount` 单位是「千元」、`vol` 是「手」，AkShare 是「元」「股」；系统已在入库时统一为「元/股」口径（`TushareProvider` 内自动 ×1000/×100），因此 `scan.min_average_amount`（成交额下限）等阈值一律按「元」填写。批量更新按标的隔离失败并重试，并以 `data.update_concurrency`（默认 2）并发拉取日线；数据源频繁报错/限流时可把该值调到 1（串行）。为避免限流导致「当日数据大面积缺失」，系统内置两道保障：① **整批重试**——单轮失败标的占比超过 `data.update_retry_failure_ratio`（默认 30%）时，自动降并发到 1 对失败标的补拉一轮；② **盘后完整性闸门**——盘后流程（`after_close`）在买点扫描与信号生成前检查「最新交易日日线覆盖率」，低于 `data.min_daily_coverage`（默认 80%）时跳过推送并改发「盘后流程推迟」告警，避免用过期收盘价误报。生产环境应先用少量代码测试：
...
- `09:26`：晨间成交——集合竞价（09:25）结束后刷新待执行标的快照，按开盘价（集合竞价结果）撮合模拟委托，并按 T+1 更新可卖数量。
...
| `lab_signal` | 因子实验室·合并模式（旧） | 读取 `signal_strategies` 表中所有「已启用」策略，合并打分生成调仓信号（**只有买点**，不建议再用）|
| `lab:<策略名>` | **因子实验室·单策略（推荐）** | 运行指定的**单条**自定义策略，独立产出买卖信号、独立执行止损止盈 |

因子窗口等参数可用 `{因子名}_{参数名}` 覆盖，例如在 `strategies` 下设置 `momentum_window: 120` 或 `macd_fast: 10`。

### 自定义策略接入模拟盘（推荐用 `lab:<策略名>`）

看板「策略」选择器已改为**多选**，选项就是你写在「因子实验室 → 策略定义」里的策略（`signal_strategies` 表）。每个策略以 `lab:<策略名>` 的形式独立运行：

- **买入**：命中该策略 `entry` 条件、且当前未被持有的标的，按「条件因子横截面 z-score 加权」排序。
- **卖出**：**只对归属本策略的持仓**（`positions.strategy == 策略名`）按其 `exit` 配置评估离场 —— 固定止损 / 固定止盈 / 移动止损 / 跌破箱体 / 离场条件，与回测共用 `lab.evaluate_exit`，**保证「回测怎么写、模拟盘就怎么执行」**。

多策略并行时的规则：

| 事项 | 规则 |
|---|---|
| 总仓位 | **共享总名额**：所有策略的买入信号合并排序后，总持仓不超过 `risk.max_positions` |
| 卖出 | 各策略的卖出信号**全部保留**（互不影响，避免漏掉止盈止损） |
| 同一标的被多策略选中 | **只买一次**，记为第一个命中它的策略 |
| 持仓归属 | 建仓时写入 `positions.strategy`（另记 `trail_peak` 移动止损峰值、`entry_breakout` 箱体基准）|
| 按策略查看 | 「委托成交」页可按策略筛选；「持仓」页展示「归属策略」列 |

> **为什么不能用 `lab_signal`**：它把表内所有策略的买入条件**合并打分**，只产出「目标组合」，
> 卖出仅靠「不再属于目标组合」——**既没有各策略独立的买卖点，也完全没有止损止盈**（回测里
> 有完整离场逻辑但模拟盘没复用），会导致「买了就不管」。多策略请用 `lab:<策略名>`。

两类策略的买入条件都支持：

- **条件类策略**（如箱体突破、主升浪、超跌反弹）：在候选池内逐股判断是否命中买入条件，命中者按「条件因子横截面 z-score 加权」打分（方向由运算符决定，`>`/`>=` 越高越好、`<`/`<=` 越低越好）。
- **内置引擎类策略**（`engine=limit_pullback_score` 涨停回马枪）：复用 `scan_limit_pullback_signals` 扫描最新交易日「涨停→冲高→回调→信号②放量阳线」形态，命中（score ≥ threshold）的信号按 `(score - threshold) / 10` 归一化后并入排序（含大盘闸门：前日涨停家数 ≥ `min_limit_up`）。
...
滚动样本外验证（`scripts/walk_forward.py`，训练段扫 threshold 60/70/80、测试段用训练最优）：四个测试段年化 +0.1% / +23.6% / +6.2% / +26.7%，样本外平均约 +14.2%（回撤约 -2.3%），**高于样本内 +8.79%**；分年度三年全正且逐年改善（+2.0% / +10.5% / +14.2%）。结论：参数稳健、当前年化未被高估，反而偏保守——但样本量偏小（尤其 2026 年仅约 2 个月），结论需随数据积累持续复核。
...
回测引擎（`ashare_quant/lab.py::run_signal_backtest`）做了两项向量化优化，用于消除全市场回测的 O(交易日数 × 股票数) 瓶颈：① **逐日循环向量化**——预构建 `entry_by_day`（日期 → 当日有入场信号的股票列表），开新仓从「每天遍历全市场 5000+ 只」改为「只遍历当日有信号的股票」，复杂度降到 O(总信号日数)；② **因子宽表计算**——抽 `_compile_factor`（`lru_cache` 缓存 AST 解析结果，避免数千次重复 `ast.parse`）与 `evaluate_factor_wide`（对每列一只股票的宽表做 pandas `shift/rolling` 向量化，一次算出全市场因子），逐股回测时只从因子宽表取列构造入场/离场掩码。实测（500 只 × 500 天）回测 3.9s → 2.2s（提速约 1.8 倍），真实策略信号更稀疏（仅 5–10%）时提速可达 10 倍以上；交易数与逐股计算完全一致（逻辑零改动）。注意停牌股因子存在微小 ffp 语义差异（宽表 `ffill` 后按自然日、逐股按交易日），停牌日不交易、占比 <1%，影响可忽略。
...
0. **今日重要事件（核心机会 + 利空风险提醒）**：只展示**当日最新**、重要性 ≥3 的核心要闻，按事件强度（★ 1~5，`magnitude`）降序，每条标注类型/方向/持续性/时间，并**关联评分最高的核心候选股票**（每事件 Top 3，关联逻辑：事件标题 + 受影响行业经产业链关键词映射到标准行业，反查候选池）。板块按方向拆两区：**① 核心机会（正/中性利好）**；**② 风险提醒（负面利空，如加息/制裁/宏观风险，单独列出提示规避）**。当**无主升浪候选**时，盘前会退而单独发一封「今日重要事件」邮件，保证每天盘前都有当日要闻。

> **事件→股票匹配的实现要点**：① 底层 `stock_industry.industry` 存的是 Tushare 原始行业名（IT设备/电器仪表/小金属 等 110 个），与 `INDUSTRY_CHAIN` 的标准 key（半导体/计算机/有色金属 等 36 个）不对应——`industry_chain.py` 的 `TUSHARE_TO_CHAIN` 映射表负责桥接。② 行业识别有三层词表：`INDUSTRY_CHAIN` 行业关键词、`CONCEPT_TO_CHAIN` 龙头公司/产品/术语（特斯拉、英伟达、OpenAI 等新闻高频词）、`MACRO_HINTS` 宏观政策/指标→受益板块（降准→银行/地产/券商）。③ 匹配按**由精确到宽松**依次尝试：事件文本直接出现 A 股公司简称 → 直接命中该股；主升浪候选池按「直接命中行业」匹配；事件专属后备池（按该事件行业查库，**按成交额**排序取代表股，每行业限额 12 只）；宏观兜底；最后才放宽到含上下游的传导匹配。

> **为什么必须「按事件单独建池」且用「直接命中行业」**：`INDUSTRY_CHAIN` 高度连通（半导体↔计算机↔通信↔化工↔电力设备 互为上下游），若用带上下游传导的 `affected_industries` 做匹配，几乎每条科技新闻都会命中「半导体」，不同事件会匹配到同一批个股；若再把所有事件的行业求并集后 `ORDER BY close DESC LIMIT 80`，等价于取全市场最贵的 80 只，会让高价股垄断所有事件的名额。因此：**匹配场景用 `direct_industries`（不传导），排序用 `amount`（成交额）而非 `close`（股价），池子按事件分别建**。`affected_industries`（含传导）仅用于「个股事件分」这类聚合评分。
...
输出各策略与组合的年化、最大回撤、夏普对比及策略间相关性矩阵。参考结论（2024-01~2026-08 全市场，**已纳入退市股修复幸存者偏差**）：涨停回马枪（年化 8.8%、回撤 5.6%、夏普 1.51）最稳健；低波动策略在修复前年化 +14.1%，修复后骤降到 +1.2%——**其 82.3% 的持仓实际是停牌/横盘的退市股**，所谓「低波动高收益」是幸存者偏差造成的假象；纯动量/趋势/反转等追涨因子普遍 -42%~-69%，短期反转甚至 -69%。核心教训：**只有主动排除 ST/退市股的策略（涨停回马枪）经得起真实全市场检验**。
