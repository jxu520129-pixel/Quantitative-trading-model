...
> **全市场性能说明**：候选池扩展为全市场（约 5000+ 只）后，SQLite 历史库达 1GB+。为控制扫描/查询耗时，系统做了四处针对性优化：① `load_bars_many` 去掉冗余 `ORDER BY`（主键 `(code, trade_date)` 索引已保证有序，此前全量排序慢约 8 倍）；② 数据库连接加大页缓存（`PRAGMA cache_size` 50MB），缓解大库读盘；③ 买点扫描与信号生成只加载最近 260 个交易日（`DataService.lookback_start`，52 周新高因子够用），全市场日线加载从约 4 分钟降到 15 秒内；④ `load_bars_many` 单连接循环 + `pd.read_sql_query` 直读（跳过 `sqlite3.Row→dict→DataFrame` 的 Python 中间层），全市场加载再提速约 40%（冷启动约 9.6s、热缓存约 5.4s）。
...
> **事件→股票匹配的两个实现要点**：① 底层 `stock_industry.industry` 存的是 Tushare 原始行业名（IT设备/电器仪表/小金属 等 110 个），与 `INDUSTRY_CHAIN` 的 17 个标准 key（半导体/计算机/有色 等）不对应——`industry_chain.py` 的 `TUSHARE_TO_CHAIN` 映射表负责桥接，否则大部分事件行业匹配不到股票；② 当主升浪候选池为空（形态未完成日）或某事件在候选池无匹配时，会**回退到数据库**按事件行业（经映射）查活跃股票（前 80 只，按收盘价排序）作为后备池，保证事件总能关联到对应行业标的。
