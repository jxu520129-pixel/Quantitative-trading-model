...
0. **今日重要事件 Top N（按重要性排序）**：只展示**当日最新**财经要闻，按事件强度（★ 1~5，`magnitude`）降序，每条标注类型/方向（正/负/中性）/持续性/时间，并**列出命中该事件行业的候选股票**（关联逻辑：事件标题 + 受影响行业经产业链关键词映射到标准行业，反查候选池）。当**无主升浪候选**时，盘前会退而单独发一封「今日重要事件」邮件，保证每天盘前都有当日要闻。
